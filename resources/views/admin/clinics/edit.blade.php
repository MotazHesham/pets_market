    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.clinics.update", [$clinic->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="user_id">{{ trans('cruds.clinic.fields.user') }}</label>
            ...
            <div class="form-group">
                <label for="phone">{{ trans('cruds.clinic.fields.phone') }}</label>
                <input class="form-control {{ $errors->has('phone') ? 'is-invalid' : '' }}" type="text" name="phone" id="phone" value="{{ old('phone', $clinic->phone) }}">
                @if($errors->has('phone'))
                    <div class="invalid-feedback">
                        {{ $errors->first('phone') }}
                    </div>
            ...
            <div class="form-group">
                <label for="banner">{{ trans('cruds.clinic.fields.banner') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.clinic.fields.banner_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="description">{{ trans('cruds.clinic.fields.description') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $clinic->description) !!}</textarea>
                @if($errors->has('description'))
                    <div class="invalid-feedback">
                        {{ $errors->first('description') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.clinic.fields.description_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="pet_types">{{ trans('cruds.clinic.fields.pet_type') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('pet_types') ? 'is-invalid' : '' }}" name="pet_types[]" id="pet_types" multiple required>
                    @foreach($pet_types as $id => $pet_type)
                        <option value="{{ $id }}" {{ (in_array($id, old('pet_types', [])) || $clinic->pet_types->contains($id)) ? 'selected' : '' }}>{{ $pet_type }}</option>
                    @endforeach
                </select>
                @if($errors->has('pet_types'))
                    <div class="invalid-feedback">
                        {{ $errors->first('pet_types') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.clinic.fields.pet_type_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    ...
    init: function () {
@if(isset($clinic) && $clinic->banner)
      var file = {!! json_encode($clinic->banner) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
      file.previewElement.classList.add('dz-complete')
